...
namespace Restify
{
    public class SpotifyTestClient : IDisposable
    {
        ...
        {
            string appkey_path;
            if (!FileSystemManager.Current.FindFileName("spotify_appkey.key", out appkey_path))
        ...
        public void Login()
        {
            lock (session)
            {
                ...
                if (!FileSystemManager.Current.FindFileName("spotify_user", out spotify_user_path))
